...
> Watertight solids, correct ISO geometry, FDM-ready profiles, a lead-in chamfer, and a tap tool ready to subtract from any solid. No manual profiling, no broken meshes, no workarounds.

Parametric generator for **threaded rods**, **hex nuts** and **taps** in SketchUp, optimised for FDM 3D printing. Compatible with SketchUp 2014 through the latest version.
...
| 2 | **D** | Nominal diameter — M3–M32 dropdown (ISO) or free input |
| 3 | **Pitch P** | Auto-filled from ISO table, always editable |
| 4 | **Parts** | ☑ Threaded rod  ☑ Hex nut  ☑ Tap (combinable) |
| 5 | **Rod height** | Length of the threaded rod |
| 6 | **Nut height** | Height of the hex nut |
| 6 | **Tap height** | Length of the threaded bore section of the tap |
| 7 | **Gap** | Radial clearance added to nut/tap bore (default 0.3 mm ISO / 0.4 mm plastic) |
...
Generated by boolean subtract (hex prism − threaded bore rod):
- Hexagonal outer shape DIN 934, M3–M32 table built-in
- Threaded bore: `r_bore = r_thread ± gap` (gap applied to radius)
- Watertight solid, no parasitic edges on top/bottom faces

**Hex size by mode:**

| Mode | Rule | Example D=10 |
|------|------|-------------|
| ISO (M3–M32 preset) | DIN 934 table value | M10 → s=16 mm |
| ISO custom D | Nearest ISO ≥ D, else 1.75×D | D=11 → M12 s=18 mm |
| FDM plastic | ISO size ≥ D, else **2×D** | D=10 → M10 s=16 mm |

## Tap

Generated by boolean union of three solids:
- **Threaded bore rod**: internal thread profile (r_bore = r_thread + gap). Length auto-extended to compensate for the entry cone.
- **45° entry cone**: full solid cone (apex at centre, base at r_bore_max height) — creates the characteristic pointed tip, cuts to the centreline.
- **Relief cylinder**: diameter slightly under the thread minor diameter (`r_minor × 0.95`), height = D.
- **Square drive**: inscribed in the relief cylinder (`side = r_cyl / √2 × 0.9`), height = `0.45 × D` — proportions per ISO 529.

The tap is a **single solid** ready to subtract from any volume to create a threaded hole. To assign a colour, select any coloured object in SketchUp before clicking Generate — the tap inherits its material.

Requires SketchUp Pro or **Eneroth Solid Tools** for the boolean union. Without them, three separate named groups are created (Thread / Shank / Square) — use the Thread group as the subtraction tool.
...
- **Nut**: single hourglass solid subtracted in one operation — chamfers both faces simultaneously, no cascading boolean.
...
| 2 | **D** | Diamètre nominal — dropdown M3–M32 (ISO) ou saisie libre |
| 3 | **Pas P** | Auto-rempli depuis la table ISO, toujours éditable |
| 4 | **Pièces** | ☑ Tige filetée  ☑ Écrou hexagonal  ☑ Tap (combinables) |
| 5 | **Hauteur tige** | Longueur de la tige filetée |
| 6 | **Hauteur écrou** | Hauteur de l'écrou hexagonal |
| 6 | **Hauteur tap** | Longueur de la section filetée du taraud |
| 7 | **Gap** | Jeu radial ajouté à l'alésage de l'écrou/taraud (défaut 0,3 mm ISO / 0,4 mm plastique) |
...
Généré par soustraction booléenne (prisme hex − bore rod fileté) :
- Forme extérieure DIN 934, table M3–M32 intégrée
- Alésage fileté : `r_alésage = r_filet ± gap` (gap appliqué au rayon)
- Solide watertight, pas d'arêtes parasites sur les faces top/bottom

**Taille hex selon le mode :**

| Mode | Règle | Exemple D=10 |
|------|-------|-------------|
| ISO preset M3–M32 | Table DIN 934 | M10 → s=16 mm |
| ISO saisie libre | ISO ≥ D le plus proche, sinon 1,75×D | D=11 → M12 s=18 mm |
| FDM plastique | ISO ≥ D, sinon **2×D** | D=10 → M10 s=16 mm |

## Taraud (Tap)

Généré par union booléenne de trois solides :
- **Bore rod fileté** : profil alésage interne, longueur allongée automatiquement pour compenser le cône d'entrée.
- **Cône d'entrée 45°** : cône plein apex au centre — coupe jusqu'au centre en z=0, crée la pointe caractéristique.
- **Cylindre de dégagement** : rayon légèrement sous le diamètre intérieur du filet (`r_minor × 0,95`), hauteur = D.
- **Prisme carré** : inscrit dans le cylindre (`côté = r_cyl / √2 × 0,9`), hauteur = `0,45 × D`.

Le taraud est un **solide unique** prêt à soustraire de n'importe quel volume. Pour lui assigner une couleur, sélectionner un objet coloré dans SketchUp avant de cliquer Générer.

Nécessite SketchUp Pro ou **Eneroth Solid Tools** pour l'union booléenne. Sans eux, trois groupes séparés sont créés (Thread / Shank / Square).

## Limitations connues

- Filet à droite uniquement
- Pas de tête de vis (DIN 933) — écrou DIN 934 uniquement
- Chanfrein et union tap : nécessite SketchUp Pro ou le plugin **Eneroth Solid Tools**
